<?php 
require_once "head.php";
require_once "body.php";
?>

<!-- Breadcrumbs-->
<ol class="breadcrumb">
            <li class="breadcrumb-item">
              <a></a>Lista de favoritos</a>
            </li>
            <li class="breadcrumb-item active">Lista de amistades</li>
        </ol>

            <!-- Icon Cards-->
        <div class="row">
            <div class="col-xl-3 col-sm-6 mb-3">
              <div class="card text-black bg-white o-hidden h-100">
                <div class="card-body">
                  <div class="card-body-icon">
                    <div class="author"><img class="rounded-circle" src="images/2079821.png" width="100" height="100"/></div>
                  </div>
                  <div class="mr-5">Negocio promo 1</div>
                  <h4>Titulo promo 1</h4>
                  <h6>Descripcion promo 1</h6>
                </div>
                <a class="card-footer text-vanilla bg-dark clearfix small z-1" href="#">
                  <span class="float-left">Ver perfil</span>
                  <span class="float-right">
                    <i class="fas fa-angle-right"></i>
                  </span>
                </a>
              </div>
            </div>
            <div class="col-xl-3 col-sm-6 mb-3">
              <div class="card text-black bg-white o-hidden h-100">
                <div class="card-body">
                  <div class="card-body-icon">
                    <div class="author"><img class="rounded-circle" src="images/2079821.png" width="100" height="100"/></div>
                  </div>
                  <div class="mr-5">Negocio promo 2</div>
                  <h4>Titulo promo 2</h4>
                  <h6>Descripcion promo 2</h6>
                </div>
                <a class="card-footer text-vanilla bg-dark clearfix small z-1" href="#">
                  <span class="float-left">Ver perfil</span>
  
                  <span class="float-right">
                    <i class="fas fa-angle-right"></i>
                  </span>
                </a>
              </div>
            </div>
            <div class="col-xl-3 col-sm-6 mb-3">
              <div class="card text-black bg-white o-hidden h-100">
                <div class="card-body">
                  <div class="card-body-icon">
                    <div class="author"><img class="rounded-circle" src="images/2079821.png" width="100" height="100"/></div>
                  </div>
                  <div class="mr-5">Negocio ads 1</div>
                  <h4>Titulo ads 1</h4>
                  <h6>Descripcion ads 1</h6>
                </div>
                <a class="card-footer text-vanilla bg-dark clearfix small z-1" href="#">
                  <span class="float-left">Ver perfil</span>
                  <span class="float-right">
                    <i class="fas fa-angle-right"></i>
                  </span>
                </a>
              </div>
            </div>
            <div class="col-xl-3 col-sm-6 mb-3">
              <div class="card text-black bg-white o-hidden h-100">
                <div class="card-body">
                  <div class="card-body-icon">
                    <div class="author"><img class="rounded-circle" src="images/2079821.png" width="100" height="100"/></div>
                  </div>
                  <div class="mr-5">Negocio ads 2</div>
                  <h4>Titulo ads 2</h4>
                  <h6>Descripcion ads 2</h6>
                </div>
                <a class="card-footer text-vanilla bg-dark clearfix small z-1" href="#">
                  <span class="float-left">Ver perfil</span>
                  <span class="float-right">
                    <i class="fas fa-angle-right"></i>
                  </span>
                </a>
              </div>
            </div>
          </div>
        </div>

<?php
require_once "footer.php";
?>